# Function that implements a conventional tail for analysis in AVL

import numpy as np
import avlwrapper as avl
import math

def Conv_tail(CASE,S_Htail,S_Vertail,cnt,neg_val,fuse_L,fuse_D):
    # Wing calcs
    ...
    Vertail_root_chord = (2*S_Vertail)/(b_Vertail*(1+Vertail_taper))
    ...
    Vertail_root_le_pnt = avl.Point(x=fuse_L -(fuse_D/2)*np.tan(math.radians((30+60+65)/3)) - Vertail_root_chord, # Places the tail
                                    y=0,
                                    z=0)
    ...
    Vertail_tip_le_pnt = avl.Point(x=fuse_L -(fuse_D/2)*np.tan(math.radians((30+60+65)/3)) - Vertail_root_chord,
                                    y=0,
                                    z=b_Vertail/2)
