# Function that implements an inverted vtail for analysis in AVL

import numpy as np
import avlwrapper as avl
import math

def Inv_Vtail(CASE,S_Vtail,S_Htail,S_Vertail,cnt,neg_val,fuse_L,fuse_D):
    # Wing calcs
    ...
    Vtail_root_le_pnt = avl.Point(x=fuse_L -(fuse_D/2)*np.tan(math.radians((30+60+65)/3)) - Vtail_tip_chord, # Places the tail
                                    y=0,
                                    z=0)
    ...
    Vtail_tip_le_pnt = avl.Point(x=fuse_L -(fuse_D/2)*np.tan(math.radians((30+60+65)/3)) - Vtail_tip_chord,
                                    y=b_Vtail/2,
