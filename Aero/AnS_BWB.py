...
def bwbaero(CASE, mach, rho, mu, v, g0, CGx, Dshell, lb):
    # # Wing Inputs
    ...
    # # Stability and Control surface Inputs
    # S_Htail = CASE.Ht_S[0]
    ...
    # # Fuselage Inputs
    # fuse_length = CASE.Fuse_L[0] # Going to use this as centerline chord length
    ...
    [wing_root_chord, wing_tip_chord, wing_taper, wing_root_le_pnt, wing_tip_le_pnt, wing_area, 
     wing_span, wing, wing_c_bar, fuse_depth, wing_root_xtc, wing_root_tc, wing_le_sweep, b] = AVLwing(CASE)
    ...
    while converged == 0:
        ...
        # Vtail_root_le_pnt = avl.Point(x=wing_c_bar + b*lb, # Places the tail at the end of the boom
        #                               y=0,
        #                               z=0)
        ...
        # Vtail_tip_le_pnt = avl.Point(x=wing_c_bar + b*lb,
        #                                y=b_Vtail/2,
        ...
        [Vtail, Vtail_mac, b_Vtail, Vtail_tip_le_pnt, Vtail_root_chord, cnt, neg_val] = Inv_Vtail(CASE,S_Vtail,S_Htail,S_Vertail,cnt,neg_val)
        ...
        l = wing_c_bar  # Have to include value for fuselage length due to skin friction def. Making it this small for flying wing
        d = fuse_depth  # Same as above. Minute diameter of fuselage
        # Define path to the TitanAtm output

        # Calculate wetted areas
        SwVtail = 2.003*S_Vtail # Tail wetted area
        bwb_fwS = 0.252 # Blended wing body fuselage portion area to wing area
        fuse_area = bwb_fwS*CASE.Wing_S[0] # Fuselage reference area
        Swf = fuse_area*(1.977+0.52*wing_root_xtc) # Fuselage wetted area for bwb (Need to update wing_root_xtc with a centerline t/c)
        SwWing = (wing_area-fuse_area)*(1.977+0.52*wing_root_xtc) # Wing wetted area
        ...
        cd0 = bwb_parasitic_drag(rho, v, mu, wing_root_tc, wing_span, wing_mac, wing_le_sweep, Vtail_mac, 0.08, l, d, lb, k, wing_area, mach, wing_root_xtc,
                                SwWing, SwVtail, Swf)
        ...
        grad_descent['S_Vtail'].append(S_Vtail)
        grad_descent['Cnb'].append(meanCnb)
        if i == 0 or neg_val == 1:
            if meanCnb >= 0.095:
                S_Vtail -= min([step, Scale * (meanCnb - (YawRange[1] + YawMid) * 0.5)])
            elif meanCnb <= 0.065:
                S_Vtail += min([step, Scale * (0.5 * (YawRange[0] + YawMid) - meanCnb)])
        else:
            xs = grad_descent['S_Vtail'][-2:]
            ys = grad_descent['Cnb'][-2:]
            slope = (ys[1]-ys[0])/(xs[1]-xs[0])
            S_Vtail = (YawMid-ys[1])/slope + xs[1]
        i += 1
        
        b_vtail_check = 0
        if b_Vtail > 0.75*b: #If the tail becomes too big, break out of the loop
            converged = 1
            print('Failure: V-tail span greater than Wing span')
            yaw_check = 0
            b_vtail_check = 1
        ...
        if b_Vtail > Dshell * 2:
            converged = 1
            yaw_check = 0
            print('Failure: V-tail Span Exceeds Constraint')
        if i > 45:
            converged = 1
            yaw_check = -1
    ...
    return (dp, S_Vtail, Vtail_tip_le_pnt.x + Vtail_root_chord * 0.666, SM, npt, struc, yaw_check, pitch_check,b_vtail_check)