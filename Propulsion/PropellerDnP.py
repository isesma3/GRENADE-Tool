...
def calculate_propeller_diameter(Data):
    ...
    #Assumed Parameters
    Mlim = 0.85 # Limit Mach Number for propeller tip
    RPM = 2700  # RPM of prop
    if nBlade == 2:
        K = 0.56
    elif nBlade == 3:
        K = 0.52
    ...
    P_BHP = Preq - Ppay
    D1 = np.sqrt((np.square(a*Mlim) - np.square(v_stream))/(math.pi*RPM))   # Compressibility-limited diameters
    D2 = K*np.sqrt(P_BHP)   # Power-regressed diameters
    return min([D1,D2]) # return the smaller diameter

def calculate_propeller_pitch(V_ktas,RPM):
    '''
    Inputs:
        V_ktas
        RPM
